...
class VibrationManager: ObservableObject {
    ...
    func getNextVibration() -> (id: Int, name: String)? {
        guard !vibrations.isEmpty else { return nil }
        if currentIndex >= vibrations.count {
            currentIndex = 0 // 🔁 boucle au début
        }
        let vibration = vibrations[currentIndex]
        currentIndex += 1
        return (id: vibration.id, name: vibration.name)
    }
    
    
    func getAllVibrations() -> [(id: Int, name: String)] {
        return vibrations.map { ($0.id, $0.name) }
    }
    
    func replayVibration(id: Int) {
